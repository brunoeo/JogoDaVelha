...
public class JogoDaVelha extends JFrame{
	...
	JButton[] bt = new JButton[9];//criando 9 botões
	boolean xo = false;//variavel booleana para trocar os turnos
	boolean[] click = new boolean [9];
	
	public JogoDaVelha() {
		setVisible(true);//janela visivel
		setTitle("Jogo da Velha");//titulo da janela
		setDefaultCloseOperation(3);//garante que a operação da janela vai fechar após clicar no X
		setLayout(null);//null pq o Layout será definido ainda 
		setBounds(250,100,400,450);//posição/tamanho
		...
		for (int i = 0; i<3; i++) {
			for(int j = 0; j<3; j++) {
				bt[cont] = new JButton();
				add(bt[cont]);
				bt[cont].setBounds((100*i)+50, (100*j)+50, 95, 95);//colocando os botões na tela
	...
	public void ganhou() {
		...
		if((bt[0].getText() == "X" && bt[1].getText() == "X" && bt[2].getText() == "X")
			...
			|| bt[2].getText() == "X" && bt[5].getText() == "X" && bt[6].getText() == "X") {
			...
			limpar();
				...
				|| bt[2].getText() == "O" && bt[5].getText() == "O" && bt[6].getText() == "O") {
